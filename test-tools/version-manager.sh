...
load_version() {
  if [ -f "$VERSION_FILE" ]; then
    ...
    PRIMIHUB_PLATFORM_VERSION="2.0.0"
...
PRIMIHUB_PLATFORM_VERSION=$PRIMIHUB_PLATFORM_VERSION
PRIMIHUB_SDK_VERSION=$PRIMIHUB_SDK_VERSION
BUILD_DATE=$BUILD_DATE
GIT_COMMIT=$GIT_COMMIT
GIT_BRANCH=$GIT_BRANCH
ARCHITECTURE=privacy0/1/2 (multi-tenant)
DB_TABLES=67+74 per tenant
EOF
  echo "✓ Version $PRIMIHUB_PLATFORM_VERSION saved"
}
...
show_version() {
  ...
  echo "=========================================="
  echo "  版本:      $PRIMIHUB_PLATFORM_VERSION"
...
docker_tag() {
  load_version
  local ver="$PRIMIHUB_PLATFORM_VERSION"
...
build_image() {
  ...
  echo "构建 primihub-platform:$PRIMIHUB_PLATFORM_VERSION ..."
  ...
  # Build Docker image
  docker build -t "primihub-platform:$PRIMIHUB_PLATFORM_VERSION" -f Dockerfile .
  docker tag "primihub-platform:$PRIMIHUB_PLATFORM_VERSION" primihub-platform:latest
  
  echo "✓ Docker镜像构建完成: primihub-platform:$PRIMIHUB_PLATFORM_VERSION"
  echo "  大小: $(docker images primihub-platform:$PRIMIHUB_PLATFORM_VERSION --format '{{.Size}}')"
}
