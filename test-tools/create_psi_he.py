...
_parser = argparse.ArgumentParser(description='创建HE PSI任务', add_help=False)
...
def create_psi_task(token, user_id):
    ...
    psi_params = {
        "taskName": f"PSI_HE全同态加密_{timestamp_str}",
        "taskDesc": "基于全同态加密(HE)算法的隐私集合求交",
        "projectId": "3",
        ...
        # 结果配置
        "resultName": f"psi_result_HE_{timestamp_str}",
...
# 创建PSI任务
print("\n【步骤2】创建PSI任务")
print("算法类型: HE (Homomorphic Encryption 全同态加密)")
print("算法特点: 允许在加密数据上直接进行计算，无需解密")
