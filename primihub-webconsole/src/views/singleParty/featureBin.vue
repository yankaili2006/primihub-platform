<template>
  <div class="app-container">
    <el-page-header content="单方特征分箱" style="margin-bottom:20px;" @back="$router.go(-1)" />
    <el-card>
      <div slot="header" style="display:flex;justify-content:space-between;align-items:center;">
        <span>特征分箱任务列表</span>
        <el-button type="primary" icon="el-icon-plus" size="small" @click="showCreate=true">新建分箱任务</el-button>
      </div>
      ...
      <el-table v-loading="loading" :data="list" border>
        ...
        <el-table-column prop="binMethod" label="分箱方法" width="140">
          <template slot-scope="{row}"><el-tag size="small">{{ methodLabel(row.binMethod) }}</el-tag></template>
        </el-table-column>
        <el-table-column prop="binCount" label="分箱数量" width="90" />
    ...
    <el-dialog title="新建特征分箱任务" :visible.sync="showCreate" width="560px" @close="resetForm">
      <el-form ref="form" :model="form" :rules="rules" label-width="110px">
        ...
        <el-form-item label="分箱方法" prop="binMethod">
          <el-select v-model="form.binMethod" placeholder="请选择分箱方法" style="width:100%;">
            <el-option v-for="m in binMethods" :key="m.value" :label="m.label" :value="m.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="分箱字段" prop="binFields">
          <el-select v-model="form.binFields" multiple placeholder="请选择分箱字段" style="width:100%;">
            <el-option v-for="f in fieldList" :key="f" :label="f" :value="f" />
          </el-select>
        </el-form-item>
        <el-form-item label="分箱数量" prop="binCount">
          <el-input-number v-model="form.binCount" :min="1" :max="20" style="width:160px;" />
          <span style="margin-left:8px;color:#999;">（1-20）</span>
        </el-form-item>
    ...
    <el-dialog :title="viewRow.taskName || '任务详情'" :visible.sync="showView" width="560px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="分箱方法">{{ methodLabel(viewRow.binMethod) }}</el-descriptions-item>
        <el-descriptions-item label="分箱数量">{{ viewRow.binCount }}</el-descriptions-item>
        <el-descriptions-item label="状态">{{ stateLabel(viewRow.taskState) }}</el-descriptions-item>
        <el-descriptions-item label="分箱字段" :span="2">{{ (viewRow.binFields || []).join('、') }}</el-descriptions-item>
...
export default {
  ...
  data() {
    return {
      query: { taskName: '', taskState: null, pageNo: 1, pageSize: 10, preprocessType: 'FEATURE_BIN' },
      list: [], total: 0, loading: false,
      binMethods: BIN_METHODS,
      statusOptions: Object.entries(STATUS_MAP).map(([v, o]) => ({ value: +v, label: o.label })),
      showCreate: false, submitting: false,
      form: { taskName: '', resourceId: '', binMethod: '', binFields: [], binCount: 5, remark: '', preprocessType: 'FEATURE_BIN' },
      rules: {
        taskName: [{ required: true, message: '请输入任务名称', trigger: 'blur' }],
        resourceId: [{ required: true, message: '请选择数据资源', trigger: 'change' }],
        binMethod: [{ required: true, message: '请选择分箱方法', trigger: 'change' }],
        binFields: [{ required: true, type: 'array', min: 1, message: '请至少选择一个分箱字段', trigger: 'change' }],
        binCount: [{ required: true, message: '请输入分箱数量', trigger: 'blur' }]
      },
      resourceList: [],
      fieldList: ['age', 'score', 'income', 'amount', 'duration', 'balance'],
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.form = { taskName: '', resourceId: '', binMethod: '', binFields: [], binCount: 5, remark: '', preprocessType: 'FEATURE_BIN' }
    },
    methodLabel(v) { return BIN_METHODS.find(m => m.value === v)?.label || v || '-' },
    ...
    async handleDownload(row) {
      try {
        ...
        const a = document.createElement('a'); a.href = url; a.download = `${row.taskName}_分箱结果.csv`; a.click()
