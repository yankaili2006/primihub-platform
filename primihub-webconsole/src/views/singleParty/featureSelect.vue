<template>
  <div class="app-container">
    <el-page-header content="单方特征筛选" style="margin-bottom:20px;" @back="$router.go(-1)" />
    <el-card>
      <div slot="header" style="display:flex;justify-content:space-between;align-items:center;">
        <span>特征筛选任务列表</span>
        <el-button type="primary" icon="el-icon-plus" size="small" @click="showCreate=true">新建筛选任务</el-button>
      </div>
      ...
      <el-table v-loading="loading" :data="list" border>
        ...
        <el-table-column prop="selectMethod" label="筛选方法" width="180">
          <template slot-scope="{row}"><el-tag size="small">{{ methodLabel(row.selectMethod) }}</el-tag></template>
        </el-table-column>
        <el-table-column prop="keepCount" label="保留特征数" width="100" />
    ...
    <el-dialog title="新建特征筛选任务" :visible.sync="showCreate" width="600px" @close="resetForm">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="筛选方法" prop="selectMethod">
          <el-select v-model="form.selectMethod" placeholder="请选择筛选方法" style="width:100%;">
            <el-option v-for="m in selectMethods" :key="m.value" :label="m.label" :value="m.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="目标字段(Label)" prop="labelField">
          <el-select v-model="form.labelField" placeholder="请选择目标字段" style="width:100%;">
            <el-option v-for="f in fieldList" :key="f" :label="f" :value="f" />
          </el-select>
        </el-form-item>
        <el-form-item label="特征字段" prop="featureFields">
          <el-select v-model="form.featureFields" multiple placeholder="请选择特征字段" style="width:100%;">
            <el-option v-for="f in fieldList" :key="f" :label="f" :value="f" />
          </el-select>
        </el-form-item>
        <el-form-item label="保留特征数量" prop="keepCount">
          <el-input-number v-model="form.keepCount" :min="1" :max="100" style="width:160px;" />
        </el-form-item>
    ...
    <el-dialog :title="viewRow.taskName || '任务详情'" :visible.sync="showView" width="560px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="筛选方法">{{ methodLabel(viewRow.selectMethod) }}</el-descriptions-item>
        <el-descriptions-item label="目标字段">{{ viewRow.labelField }}</el-descriptions-item>
        <el-descriptions-item label="保留特征数">{{ viewRow.keepCount }}</el-descriptions-item>
        <el-descriptions-item label="特征字段" :span="2">{{ (viewRow.featureFields || []).join('、') }}</el-descriptions-item>
...
const SELECT_METHODS = [
  { value: 'VARIANCE', label: '方差过滤' },
  { value: 'CORRELATION', label: '相关系数过滤' },
  { value: 'CHI_SQUARE', label: '卡方检验' },
  { value: 'RFE', label: '递归特征消除（RFE）' },
  { value: 'L1', label: 'L1正则化筛选' }
]
...
export default {
  ...
  data() {
    return {
      query: { taskName: '', taskState: null, pageNo: 1, pageSize: 10, preprocessType: 'FEATURE_SELECT' },
      list: [], total: 0, loading: false,
      selectMethods: SELECT_METHODS,
      statusOptions: Object.entries(STATUS_MAP).map(([v, o]) => ({ value: +v, label: o.label })),
      showCreate: false, submitting: false,
      form: { taskName: '', resourceId: '', selectMethod: '', labelField: '', featureFields: [], keepCount: 10, remark: '', preprocessType: 'FEATURE_SELECT' },
      rules: {
        taskName: [{ required: true, message: '请输入任务名称', trigger: 'blur' }],
        resourceId: [{ required: true, message: '请选择数据资源', trigger: 'change' }],
        selectMethod: [{ required: true, message: '请选择筛选方法', trigger: 'change' }],
        labelField: [{ required: true, message: '请选择目标字段', trigger: 'change' }],
        featureFields: [{ required: true, type: 'array', min: 1, message: '请至少选择一个特征字段', trigger: 'change' }],
        keepCount: [{ required: true, message: '请输入保留特征数量', trigger: 'blur' }]
      },
      resourceList: [],
      fieldList: ['label', 'age', 'score', 'income', 'feature1', 'feature2', 'feature3', 'amount'],
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.form = { taskName: '', resourceId: '', selectMethod: '', labelField: '', featureFields: [], keepCount: 10, remark: '', preprocessType: 'FEATURE_SELECT' }
    },
    methodLabel(v) { return SELECT_METHODS.find(m => m.value === v)?.label || v || '-' },
    ...
    async handleDownload(row) {
      try {
        ...
        const a = document.createElement('a'); a.href = url; a.download = `${row.taskName}_筛选结果.csv`; a.click()
