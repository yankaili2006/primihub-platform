<template>
  <div class="app-container">
    <el-page-header content="单方Python脚本处理" style="margin-bottom:20px;" @back="$router.go(-1)" />
    <el-card>
      <div slot="header" style="display:flex;justify-content:space-between;align-items:center;">
        <span>Python脚本任务列表</span>
        <el-button type="primary" icon="el-icon-plus" size="small" @click="showCreate=true">新建脚本任务</el-button>
      </div>
      ...
      <el-table v-loading="loading" :data="list" border>
        ...
        <el-table-column prop="outputVar" label="输出变量" width="130" />
    ...
    <el-dialog title="新建Python脚本任务" :visible.sync="showCreate" width="680px" @close="resetForm">
      <el-form ref="form" :model="form" :rules="rules" label-width="110px">
        ...
        <el-form-item label="Python脚本" prop="scriptContent">
          <el-input
        ...
        <el-form-item label="输出变量名" prop="outputVar">
          <el-input v-model="form.outputVar" placeholder="脚本中结果DataFrame的变量名，如 result_df" />
        </el-form-item>
    ...
    <el-dialog :title="viewRow.taskName || '任务详情'" :visible.sync="showView" width="680px">
      <el-descriptions :column="2" border style="margin-bottom:12px;">
        ...
        <el-descriptions-item label="输出变量">{{ viewRow.outputVar }}</el-descriptions-item>
      ...
      <div style="font-weight:600;margin-bottom:6px;">脚本内容</div>
      <pre style="background:#f5f7fa;padding:12px;border-radius:4px;font-size:13px;overflow:auto;max-height:300px;">{{ viewRow.scriptContent }}</pre>
      <div slot="footer">
...
const DEFAULT_SCRIPT = `# 使用 pandas 处理数据
import pandas as pd

# df 为输入数据 DataFrame
result_df = df.copy()
result_df['new_col'] = result_df['col1'] * 2`

export default {
  name: 'SinglePartyPythonScript',
  data() {
    return {
      query: { taskName: '', taskState: null, pageNo: 1, pageSize: 10, preprocessType: 'PYTHON_SCRIPT' },
      list: [], total: 0, loading: false,
      statusOptions: Object.entries(STATUS_MAP).map(([v, o]) => ({ value: +v, label: o.label })),
      showCreate: false, submitting: false,
      form: { taskName: '', resourceId: '', scriptContent: DEFAULT_SCRIPT, outputVar: 'result_df', remark: '', preprocessType: 'PYTHON_SCRIPT' },
      rules: {
        taskName: [{ required: true, message: '请输入任务名称', trigger: 'blur' }],
        resourceId: [{ required: true, message: '请选择数据资源', trigger: 'change' }],
        scriptContent: [{ required: true, message: '请输入Python脚本', trigger: 'blur' }],
        outputVar: [{ required: true, message: '请输入输出变量名', trigger: 'blur' }]
      },
      resourceList: [],
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.form = { taskName: '', resourceId: '', scriptContent: DEFAULT_SCRIPT, outputVar: 'result_df', remark: '', preprocessType: 'PYTHON_SCRIPT' }
    },
    ...
    async handleDownload(row) {
      try {
        ...
        const a = document.createElement('a'); a.href = url; a.download = `${row.taskName}_脚本结果.csv`; a.click()
