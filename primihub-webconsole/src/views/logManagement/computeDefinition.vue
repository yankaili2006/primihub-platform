<template>
  <div class="container">
    <div class="filter-bar">
      ...
      <el-select v-model="searchForm.computeType" placeholder="请选择计算类型" style="width: 150px; margin-right: 10px;" clearable>
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogTitle" width="600px" :before-close="closeDialog">
      <el-form ref="form" :model="formData" label-width="120px" :rules="rules">
        ...
        <el-form-item label="计算类型" prop="computeType">
          <el-select v-model="formData.computeType" placeholder="请选择计算类型" style="width: 100%;">
            <el-option label="联合建模" value="联合建模" />
            <el-option label="安全求交" value="安全求交" />
            <el-option label="隐匿查询" value="隐匿查询" />
            <el-option label="联合预测" value="联合预测" />
...
<script>
import { getComputeLogDefinitionPage, addComputeLogDefinition, updateComputeLogDefinition, deleteComputeLogDefinition } from '@/api/logManagement'
...
export default {
  ...
  data() {
    return {
      list: [],
      searchForm: { keyword: '', computeType: '' },
      dialogFlag: '',
      dialogTitle: '',
      dialogVisible: false,
      formData: {
        id: null, logCode: '', logName: '', computeType: '', moduleName: '',
      ...
      rules: {
        ...
        computeType: [{ required: true, message: '请选择计算类型', trigger: 'change' }]
      },
  ...
  methods: {
    fetchData() {
      getComputeLogDefinitionPage({ ...this.searchForm, pageNum: this.pageNum, pageSize: this.pageSize }).then(res => {
    ...
    handleReset() { this.searchForm = { keyword: '', computeType: '' }; this.pageNum = 1; this.fetchData() },
    handleAdd() {
      this.dialogFlag = 'add'; this.dialogTitle = '新增计算日志定义'
      this.formData = { id: null, logCode: '', logName: '', computeType: '', moduleName: '', description: '', retentionDays: 30, isEnabled: 1 }
      this.dialogVisible = true
    },
    handleEdit(row) { this.dialogFlag = 'edit'; this.dialogTitle = '编辑计算日志定义'; this.formData = { ...row }; this.dialogVisible = true },
    submitForm() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          const action = this.dialogFlag === 'add' ? addComputeLogDefinition : updateComputeLogDefinition
    ...
    handleDelete(row) {
      this.$confirm('确认删除该日志定义吗？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
        .then(() => {
          deleteComputeLogDefinition({ id: row.id }).then(res => {
