<template>
  <div class="app-container">
    <el-page-header content="联邦统计 - 占比统计" style="margin-bottom:20px;" @back="$router.go(-1)" />
    <el-card>
    ...
    <el-dialog title="新建占比统计任务" :visible.sync="showCreate" width="580px" @close="resetForm">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="分类字段" prop="categoryField">
          <el-input v-model="form.categoryField" placeholder="请输入分类字段名" />
        </el-form-item>
        <el-form-item label="统计字段" prop="statsField">
          <el-input v-model="form.statsField" placeholder="请输入统计字段名" />
        </el-form-item>
        <el-form-item label="占比计算方式" prop="ratioType">
          <el-radio-group v-model="form.ratioType">
            <el-radio label="ROW">行占比</el-radio>
            <el-radio label="COL">列占比</el-radio>
            <el-radio label="TOTAL">总占比</el-radio>
          </el-radio-group>
        </el-form-item>
    ...
    <el-dialog :title="viewRow.taskName || '任务详情'" :visible.sync="showView" width="560px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="分类字段">{{ viewRow.categoryField }}</el-descriptions-item>
        <el-descriptions-item label="统计字段">{{ viewRow.statsField }}</el-descriptions-item>
        <el-descriptions-item label="占比计算方式">{{ ratioTypeLabel(viewRow.ratioType) }}</el-descriptions-item>
...
const RATIO_TYPES = { ROW: '行占比', COL: '列占比', TOTAL: '总占比' }

export default {
  name: 'FederatedStatisticsRatioStats',
  data() {
    return {
      query: { taskName: '', taskStatus: null, statisticsType: 'RATIO', pageNo: 1, pageSize: 10 },
      list: [], total: 0, loading: false,
      statusOptions: Object.entries(STATUS_MAP).map(([v, o]) => ({ value: +v, label: o.label })),
      showCreate: false, submitting: false,
      form: { taskName: '', participants: [], dataResources: [], categoryField: '', statsField: '', ratioType: 'TOTAL', remark: '', statisticsType: 'RATIO' },
      rules: {
        ...
        categoryField: [{ required: true, message: '请输入分类字段', trigger: 'blur' }],
        statsField: [{ required: true, message: '请输入统计字段', trigger: 'blur' }],
        ratioType: [{ required: true, message: '请选择占比计算方式', trigger: 'change' }]
      },
      resourceList: [],
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.form = { taskName: '', participants: [], dataResources: [], categoryField: '', statsField: '', ratioType: 'TOTAL', remark: '', statisticsType: 'RATIO' }
    },
    ratioTypeLabel(v) { return RATIO_TYPES[v] || v || '-' },
    ...
    async handleDownload(row) {
      try {
        ...
        const a = document.createElement('a'); a.href = url; a.download = `统计结果_${row.taskName}_${Date.now()}.csv`
