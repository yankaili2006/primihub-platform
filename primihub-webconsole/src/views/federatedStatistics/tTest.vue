<template>
  <div class="app-container">
    <el-page-header content="联邦统计 - T检验" style="margin-bottom:20px;" @back="$router.go(-1)" />
    <el-card>
      ...
      <el-table v-loading="loading" :data="list" border>
        ...
        <el-table-column prop="testType" label="检验类型" width="180">
          <template slot-scope="{row}"><el-tag size="small">{{ testTypeLabel(row.testType) }}</el-tag></template>
        </el-table-column>
    ...
    <el-dialog title="新建T检验任务" :visible.sync="showCreate" width="580px" @close="resetForm">
      <el-form ref="form" :model="form" :rules="rules" label-width="130px">
        ...
        <el-form-item label="检验类型" prop="testType">
          <el-select v-model="form.testType" placeholder="请选择检验类型" style="width:100%;">
            <el-option label="单样本t检验" value="ONE_SAMPLE" />
            <el-option label="独立双样本t检验" value="TWO_SAMPLE" />
            <el-option label="配对t检验" value="PAIRED" />
          </el-select>
        </el-form-item>
        <el-form-item label="检验字段" prop="testField">
          <el-input v-model="form.testField" placeholder="请输入检验字段名" />
        </el-form-item>
    ...
    <el-dialog :title="viewRow.taskName || '任务详情'" :visible.sync="showView" width="560px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="检验类型">{{ testTypeLabel(viewRow.testType) }}</el-descriptions-item>
        <el-descriptions-item label="检验字段">{{ viewRow.testField }}</el-descriptions-item>
...
const TEST_TYPES = { ONE_SAMPLE: '单样本t检验', TWO_SAMPLE: '独立双样本t检验', PAIRED: '配对t检验' }

export default {
  name: 'FederatedStatisticsTTest',
  data() {
    return {
      query: { taskName: '', taskStatus: null, statisticsType: 'T_TEST', pageNo: 1, pageSize: 10 },
      list: [], total: 0, loading: false,
      statusOptions: Object.entries(STATUS_MAP).map(([v, o]) => ({ value: +v, label: o.label })),
      showCreate: false, submitting: false,
      form: { taskName: '', participants: [], dataResources: [], testType: '', testField: '', alpha: 0.05, remark: '', statisticsType: 'T_TEST' },
      rules: {
        ...
        testField: [{ required: true, message: '请输入检验字段', trigger: 'blur' }],
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.form = { taskName: '', participants: [], dataResources: [], testType: '', testField: '', alpha: 0.05, remark: '', statisticsType: 'T_TEST' }
    },
    testTypeLabel(v) { return TEST_TYPES[v] || v || '-' },
    ...
    async handleDownload(row) {
      try {
        ...
        const a = document.createElement('a'); a.href = url; a.download = `T检验_${row.taskName}_${Date.now()}.csv`
