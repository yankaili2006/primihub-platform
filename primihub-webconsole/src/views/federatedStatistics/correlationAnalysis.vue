<template>
  <div class="app-container">
    <el-page-header content="联邦统计 - 相关性分析" style="margin-bottom:20px;" @back="$router.go(-1)" />
    <el-card>
      ...
      <el-table v-loading="loading" :data="list" border>
        ...
        <el-table-column prop="correlationType" label="相关系数类型" width="160">
          <template slot-scope="{row}"><el-tag size="small">{{ correlationTypeLabel(row.correlationType) }}</el-tag></template>
        </el-table-column>
    ...
    <el-dialog title="新建相关性分析任务" :visible.sync="showCreate" width="600px" @close="resetForm">
      <el-form ref="form" :model="form" :rules="rules" label-width="130px">
        ...
        <el-form-item label="相关系数类型" prop="correlationType">
          <el-radio-group v-model="form.correlationType">
            <el-radio label="PEARSON">Pearson</el-radio>
            <el-radio label="SPEARMAN">Spearman</el-radio>
            <el-radio label="KENDALL">Kendall</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="分析字段" prop="analysisFields">
          <el-select v-model="form.analysisFields" multiple allow-create filterable placeholder="请选择或输入分析字段" style="width:100%;">
            <el-option v-for="f in fieldList" :key="f" :label="f" :value="f" />
          </el-select>
        </el-form-item>
        <el-form-item label="生成热力图">
          <el-switch v-model="form.generateHeatmap" active-text="是" inactive-text="否" />
        </el-form-item>
    ...
    <el-dialog :title="viewRow.taskName || '任务详情'" :visible.sync="showView" width="560px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="相关系数类型">{{ correlationTypeLabel(viewRow.correlationType) }}</el-descriptions-item>
        <el-descriptions-item label="生成热力图">{{ viewRow.generateHeatmap ? '是' : '否' }}</el-descriptions-item>
        <el-descriptions-item label="分析字段" :span="2">{{ (viewRow.analysisFields || []).join('、') }}</el-descriptions-item>
...
const CORRELATION_TYPES = { PEARSON: 'Pearson', SPEARMAN: 'Spearman', KENDALL: 'Kendall' }

export default {
  name: 'FederatedStatisticsCorrelationAnalysis',
  data() {
    return {
      query: { taskName: '', taskStatus: null, statisticsType: 'CORRELATION', pageNo: 1, pageSize: 10 },
      list: [], total: 0, loading: false,
      statusOptions: Object.entries(STATUS_MAP).map(([v, o]) => ({ value: +v, label: o.label })),
      showCreate: false, submitting: false,
      form: { taskName: '', participants: [], dataResources: [], correlationType: 'PEARSON', analysisFields: [], generateHeatmap: true, remark: '', statisticsType: 'CORRELATION' },
      rules: {
        ...
        correlationType: [{ required: true, message: '请选择相关系数类型', trigger: 'change' }],
        analysisFields: [{ required: true, type: 'array', min: 2, message: '请至少选择两个分析字段', trigger: 'change' }]
      },
      resourceList: [],
      fieldList: ['age', 'income', 'score', 'feature1', 'feature2', 'feature3', 'amount', 'duration'],
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.form = { taskName: '', participants: [], dataResources: [], correlationType: 'PEARSON', analysisFields: [], generateHeatmap: true, remark: '', statisticsType: 'CORRELATION' }
    },
    correlationTypeLabel(v) { return CORRELATION_TYPES[v] || v || '-' },
    ...
    async handleDownload(row) {
      try {
        ...
        const a = document.createElement('a'); a.href = url; a.download = `相关分析_${row.taskName}_${Date.now()}.csv`
