<template>
  <div class="app-container">
    <el-page-header content="联邦统计 - 回归分析" style="margin-bottom:20px;" @back="$router.go(-1)" />
    <el-card>
      ...
      <el-table v-loading="loading" :data="list" border>
        ...
        <el-table-column prop="regressionType" label="回归类型" width="160">
          <template slot-scope="{row}"><el-tag size="small">{{ regressionTypeLabel(row.regressionType) }}</el-tag></template>
        </el-table-column>
        <el-table-column prop="dependentVar" label="因变量" width="120" />
    ...
    <el-dialog title="新建回归分析任务" :visible.sync="showCreate" width="600px" @close="resetForm">
      <el-form ref="form" :model="form" :rules="rules" label-width="130px">
        ...
        <el-form-item label="回归类型" prop="regressionType">
          <el-select v-model="form.regressionType" placeholder="请选择回归类型" style="width:100%;">
            <el-option label="线性回归" value="LINEAR" />
            <el-option label="多元线性回归" value="MULTI_LINEAR" />
            <el-option label="逻辑回归" value="LOGISTIC" />
            <el-option label="岭回归" value="RIDGE" />
          </el-select>
        </el-form-item>
        <el-form-item label="因变量" prop="dependentVar">
          <el-input v-model="form.dependentVar" placeholder="请输入因变量字段名" />
        </el-form-item>
        <el-form-item label="自变量" prop="independentVars">
          <el-select v-model="form.independentVars" multiple allow-create filterable placeholder="请输入或选择自变量字段名" style="width:100%;">
            <el-option v-for="f in fieldList" :key="f" :label="f" :value="f" />
          </el-select>
        </el-form-item>
        <el-form-item label="显示系数图">
          <el-switch v-model="form.showCoefficientChart" active-text="是" inactive-text="否" />
        </el-form-item>
    ...
    <el-dialog :title="viewRow.taskName || '任务详情'" :visible.sync="showView" width="560px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="回归类型">{{ regressionTypeLabel(viewRow.regressionType) }}</el-descriptions-item>
        <el-descriptions-item label="因变量">{{ viewRow.dependentVar }}</el-descriptions-item>
        <el-descriptions-item label="自变量" :span="2">{{ (viewRow.independentVars || []).join('、') }}</el-descriptions-item>
        <el-descriptions-item label="显示系数图">{{ viewRow.showCoefficientChart ? '是' : '否' }}</el-descriptions-item>
...
const REGRESSION_TYPES = { LINEAR: '线性回归', MULTI_LINEAR: '多元线性回归', LOGISTIC: '逻辑回归', RIDGE: '岭回归' }

export default {
  name: 'FederatedStatisticsRegressionAnalysis',
  data() {
    return {
      query: { taskName: '', taskStatus: null, statisticsType: 'REGRESSION', pageNo: 1, pageSize: 10 },
      list: [], total: 0, loading: false,
      statusOptions: Object.entries(STATUS_MAP).map(([v, o]) => ({ value: +v, label: o.label })),
      showCreate: false, submitting: false,
      form: { taskName: '', participants: [], dataResources: [], regressionType: '', dependentVar: '', independentVars: [], showCoefficientChart: true, remark: '', statisticsType: 'REGRESSION' },
      rules: {
        ...
        regressionType: [{ required: true, message: '请选择回归类型', trigger: 'change' }],
        dependentVar: [{ required: true, message: '请输入因变量', trigger: 'blur' }],
        independentVars: [{ required: true, type: 'array', min: 1, message: '请至少输入一个自变量', trigger: 'change' }]
      },
      resourceList: [],
      fieldList: ['age', 'income', 'score', 'feature1', 'feature2', 'feature3'],
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.form = { taskName: '', participants: [], dataResources: [], regressionType: '', dependentVar: '', independentVars: [], showCoefficientChart: true, remark: '', statisticsType: 'REGRESSION' }
    },
    regressionTypeLabel(v) { return REGRESSION_TYPES[v] || v || '-' },
    ...
    async handleDownload(row) {
      try {
        ...
        const a = document.createElement('a'); a.href = url; a.download = `回归分析_${row.taskName}_${Date.now()}.csv`
