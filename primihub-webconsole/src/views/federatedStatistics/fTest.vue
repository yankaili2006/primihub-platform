<template>
  <div class="app-container">
    <el-page-header content="联邦统计 - F检验（方差分析）" style="margin-bottom:20px;" @back="$router.go(-1)" />
    <el-card>
      ...
      <el-table v-loading="loading" :data="list" border>
        ...
        <el-table-column prop="groupVar" label="分组变量" width="130" />
        <el-table-column prop="responseVar" label="响应变量" width="130" />
    ...
    <el-dialog title="新建F检验任务" :visible.sync="showCreate" width="580px" @close="resetForm">
      <el-form ref="form" :model="form" :rules="rules" label-width="130px">
        ...
        <el-form-item label="分组变量" prop="groupVar">
          <el-input v-model="form.groupVar" placeholder="请输入分组变量字段名（分类型）" />
        </el-form-item>
        <el-form-item label="响应变量" prop="responseVar">
          <el-input v-model="form.responseVar" placeholder="请输入响应变量字段名（连续型）" />
        </el-form-item>
    ...
    <el-dialog :title="viewRow.taskName || '任务详情'" :visible.sync="showView" width="560px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="分组变量">{{ viewRow.groupVar }}</el-descriptions-item>
        <el-descriptions-item label="响应变量">{{ viewRow.responseVar }}</el-descriptions-item>
...
export default {
  ...
  data() {
    return {
      query: { taskName: '', taskStatus: null, statisticsType: 'F_TEST', pageNo: 1, pageSize: 10 },
      list: [], total: 0, loading: false,
      statusOptions: Object.entries(STATUS_MAP).map(([v, o]) => ({ value: +v, label: o.label })),
      showCreate: false, submitting: false,
      form: { taskName: '', participants: [], dataResources: [], groupVar: '', responseVar: '', alpha: 0.05, remark: '', statisticsType: 'F_TEST' },
      rules: {
        ...
        groupVar: [{ required: true, message: '请输入分组变量', trigger: 'blur' }],
        responseVar: [{ required: true, message: '请输入响应变量', trigger: 'blur' }],
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.form = { taskName: '', participants: [], dataResources: [], groupVar: '', responseVar: '', alpha: 0.05, remark: '', statisticsType: 'F_TEST' }
    },
    ...
    async handleDownload(row) {
      try {
        ...
        const a = document.createElement('a'); a.href = url; a.download = `F检验_${row.taskName}_${Date.now()}.csv`
