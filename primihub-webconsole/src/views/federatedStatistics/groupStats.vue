<template>
  <div class="app-container">
    <el-page-header content="联邦统计 - 分组统计" style="margin-bottom:20px;" @back="$router.go(-1)" />
    <el-card>
    ...
    <el-dialog title="新建分组统计任务" :visible.sync="showCreate" width="580px" @close="resetForm">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="分组字段" prop="groupField">
          <el-input v-model="form.groupField" placeholder="请输入分组字段名" />
        </el-form-item>
        <el-form-item label="聚合函数" prop="aggFunctions">
          <el-checkbox-group v-model="form.aggFunctions">
            <el-checkbox label="SUM">SUM</el-checkbox>
            <el-checkbox label="COUNT">COUNT</el-checkbox>
            <el-checkbox label="AVG">AVG</el-checkbox>
            <el-checkbox label="MAX">MAX</el-checkbox>
            <el-checkbox label="MIN">MIN</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
    ...
    <el-dialog :title="viewRow.taskName || '任务详情'" :visible.sync="showView" width="560px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="分组字段">{{ viewRow.groupField }}</el-descriptions-item>
        <el-descriptions-item label="聚合函数">{{ (viewRow.aggFunctions || []).join('、') }}</el-descriptions-item>
...
export default {
  ...
  data() {
    return {
      query: { taskName: '', taskStatus: null, statisticsType: 'GROUP_BY', pageNo: 1, pageSize: 10 },
      list: [], total: 0, loading: false,
      statusOptions: Object.entries(STATUS_MAP).map(([v, o]) => ({ value: +v, label: o.label })),
      showCreate: false, submitting: false,
      form: { taskName: '', participants: [], dataResources: [], groupField: '', aggFunctions: ['COUNT'], remark: '', statisticsType: 'GROUP_BY' },
      rules: {
        ...
        groupField: [{ required: true, message: '请输入分组字段', trigger: 'blur' }],
        aggFunctions: [{ required: true, type: 'array', min: 1, message: '请至少选择一个聚合函数', trigger: 'change' }]
      },
      resourceList: [],
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.form = { taskName: '', participants: [], dataResources: [], groupField: '', aggFunctions: ['COUNT'], remark: '', statisticsType: 'GROUP_BY' }
    },
    ...
    async handleDownload(row) {
      try {
        ...
        const a = document.createElement('a'); a.href = url; a.download = `分组统计_${row.taskName}_${Date.now()}.csv`
