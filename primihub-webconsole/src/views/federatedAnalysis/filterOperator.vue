<template>
  <div class="app-container">
    <el-page-header content="筛选算子（WHERE/HAVING/FILTER）" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-alert
      title="筛选算子用于在联邦分析中对多方数据进行条件过滤，支持 WHERE、HAVING、FILTER 等标准 SQL 过滤语法。"
      type="info"
      :closable="false"
      show-icon
      ...
      <el-col :span="8">
        <el-card>
          ...
          <div class="syntax-doc">
            <p class="doc-title">支持的过滤算子</p>
            <ul class="doc-list">
              <li><code>WHERE</code>：行级过滤条件，在分组前应用</li>
              <li><code>HAVING</code>：分组后过滤，通常与 GROUP BY 配合使用</li>
              <li><code>FILTER</code>：窗口函数的条件过滤扩展</li>
            </ul>
            <p class="doc-title">语法格式</p>
            <pre class="doc-pre">SELECT col1, col2
FROM table
WHERE condition1
  AND condition2
GROUP BY col1
HAVING agg_condition</pre>
            <p class="doc-title">联邦场景注意事项</p>
            <ul class="doc-list">
              <li>过滤条件不会泄露原始数据</li>
              <li>跨方字段过滤需明确指定数据来源方</li>
              <li>支持子查询作为过滤条件</li>
            </ul>
          </div>
        </el-card>
...
const ANALYSIS_TYPE = 'FILTER'

const DEFAULT_SQL = `SELECT customer_id, amount
FROM sales
WHERE amount > 1000
  AND region = 'NORTH'
HAVING count(*) > 5`
