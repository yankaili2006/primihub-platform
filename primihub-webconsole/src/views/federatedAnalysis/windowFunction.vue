<template>
  <div class="app-container">
    <el-page-header content="窗口函数（OVER/PARTITION BY）" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-alert
      title="窗口函数在联邦分析中对数据分区并按顺序计算，支持 ROW_NUMBER/RANK/LAG/LEAD/SUM OVER 等函数，在不汇总行的情况下进行窗口计算。"
      type="info"
      :closable="false"
      show-icon
      ...
      <el-col :span="8">
        <el-card>
          ...
          <div class="syntax-doc">
            <p class="doc-title">窗口函数语法</p>
            <pre class="doc-pre">FUNC() OVER (
  PARTITION BY col1
  ORDER BY col2 ASC
  ROWS BETWEEN ... AND ...
)</pre>
            <p class="doc-title">支持的窗口函数</p>
            <ul class="doc-list">
              <li><code>ROW_NUMBER()</code>：分区内行号</li>
              <li><code>RANK()</code>：分区内排名（相同值同排名）</li>
              <li><code>DENSE_RANK()</code>：密集排名</li>
              <li><code>LAG(col, n)</code>：向前偏移 n 行的值</li>
              <li><code>LEAD(col, n)</code>：向后偏移 n 行的值</li>
              <li><code>SUM/AVG OVER</code>：滑动聚合</li>
            </ul>
            <p class="doc-title">ROW_NUMBER 排名示例</p>
            <pre class="doc-pre">SELECT user_id, region, amount,
  ROW_NUMBER() OVER (
    PARTITION BY region
...
const ANALYSIS_TYPE = 'WINDOW'

const DEFAULT_SQL = `SELECT user_id, region, amount,
  ROW_NUMBER() OVER (
    PARTITION BY region
    ORDER BY amount DESC
  ) AS rank_in_region,
