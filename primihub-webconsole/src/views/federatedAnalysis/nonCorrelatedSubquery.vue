<template>
  <div class="app-container">
    <el-page-header content="非关联子查询（标量/IN子查询）" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-alert
      title="非关联子查询是指子查询与外层查询相互独立，子查询只执行一次，结果供外层查询使用。支持标量子查询和 IN 子查询等形式。"
      type="info"
      :closable="false"
      show-icon
      ...
      <el-col :span="8">
        <el-card>
          ...
          <div class="syntax-doc">
            <p class="doc-title">非关联子查询类型</p>
            <ul class="doc-list">
              <li><strong>标量子查询</strong>：返回单个值，可用在 SELECT/WHERE 中</li>
              <li><strong>IN 子查询</strong>：返回值集合，用于 IN 条件判断</li>
              <li><strong>NOT IN 子查询</strong>：排除集合中的记录</li>
              <li><strong>FROM 子查询</strong>：作为派生表</li>
            </ul>
            <p class="doc-title">IN 子查询示例</p>
            <pre class="doc-pre">-- 查询高风险用户的订单
SELECT o.order_id, o.amount
FROM party_b.orders o
WHERE o.user_id IN (
  SELECT user_id
  FROM party_a.risk_users
  WHERE risk_level = 'HIGH'
)</pre>
            <p class="doc-title">标量子查询示例</p>
            <pre class="doc-pre">-- 与平均值比较
...
const ANALYSIS_TYPE = 'NON_CORRELATED_SUBQUERY'

const DEFAULT_SQL = `-- 查询高风险用户的所有订单
