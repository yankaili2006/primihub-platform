<template>
  <div class="app-container">
    <el-page-header content="关联子查询（EXISTS/IN with Correlation）" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-alert
      title="关联子查询是指子查询中引用了外层查询的字段，常见于 EXISTS 和相关 IN 子句中，可在联邦场景下实现跨方的关联过滤。"
      type="info"
      :closable="false"
      show-icon
      ...
      <el-col :span="8">
        <el-card>
          ...
          <div class="syntax-doc">
            <p class="doc-title">关联子查询特征</p>
            <ul class="doc-list">
              <li>子查询引用外层查询的字段</li>
              <li>外层每行都会触发子查询执行</li>
              <li>常用于 <code>EXISTS</code>、<code>NOT EXISTS</code></li>
            </ul>
            <p class="doc-title">EXISTS 关联子查询示例</p>
            <pre class="doc-pre">-- 查询有下单记录的用户
SELECT u.user_id, u.name
FROM party_a.users u
WHERE EXISTS (
  SELECT 1
  ...
  WHERE o.user_id = u.user_id
    ...
    AND o.status = 'PAID'
            ...
            <p class="doc-title">NOT EXISTS 示例</p>
            <pre class="doc-pre">-- 查询从未下单的用户
SELECT u.user_id, u.name
FROM users u
WHERE NOT EXISTS (
...
const ANALYSIS_TYPE = 'CORRELATED_SUBQUERY'

const DEFAULT_SQL = `-- 查询有大额订单记录的用户
SELECT u.user_id, u.name, u.region
FROM party_a.users u
WHERE EXISTS (
  SELECT 1
  FROM party_b.orders o
