<template>
  <div class="app-container">
    <el-page-header content="分组算子（GROUP BY）" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-alert
      title="分组算子用于在联邦分析中对多方数据按指定字段分组统计，支持 GROUP BY + HAVING 语法，在隐私保护前提下进行分组聚合分析。"
      type="info"
      :closable="false"
      show-icon
      ...
      <el-col :span="8">
        <el-card>
          ...
          <div class="syntax-doc">
            <p class="doc-title">GROUP BY 语法</p>
            <pre class="doc-pre">SELECT col1, AGG_FUNC(col2)
FROM table
WHERE condition
GROUP BY col1
HAVING AGG_FUNC(col2) > value
ORDER BY AGG_FUNC(col2) DESC</pre>
            <p class="doc-title">按地区分组示例</p>
            <pre class="doc-pre">SELECT region,
  COUNT(*) AS user_count,
...
ORDER BY total_amount DESC</pre>
            <p class="doc-title">使用注意事项</p>
            <ul class="doc-list">
              <li>SELECT 中非聚合字段必须出现在 GROUP BY 中</li>
              <li>HAVING 用于过滤分组结果，WHERE 用于过滤原始数据</li>
              <li>支持多字段分组：GROUP BY col1, col2</li>
            </ul>
          </div>
        </el-card>
      </el-col>
