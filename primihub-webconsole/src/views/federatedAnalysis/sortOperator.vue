<template>
  <div class="app-container">
    <el-page-header content="排序算子（ORDER BY）" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-alert
      title="排序算子用于在联邦分析中对多方数据的查询结果按指定字段排序，支持多字段排序、ASC（升序）和 DESC（降序）。"
      type="info"
      :closable="false"
      show-icon
      ...
      <el-col :span="8">
        <el-card>
          ...
          <div class="syntax-doc">
            <p class="doc-title">ORDER BY 语法</p>
            <pre class="doc-pre">SELECT col1, col2, col3
FROM table
WHERE condition
ORDER BY col1 ASC, col2 DESC
LIMIT n</pre>
            <p class="doc-title">多字段排序示例</p>
            <pre class="doc-pre">SELECT user_id, region,
  amount, create_time
...
ORDER BY
  ...
  region ASC
LIMIT 100</pre>
            <p class="doc-title">排序说明</p>
            <ul class="doc-list">
              <li><code>ASC</code>：升序排列（默认）</li>
              <li><code>DESC</code>：降序排列</li>
              <li>多字段排序按字段声明顺序依次排列</li>
              <li>联邦排序结果在安全计算后汇总</li>
              <li>建议配合 LIMIT 使用以提高性能</li>
            </ul>
          </div>
        </el-card>
      </el-col>
...
const ANALYSIS_TYPE = 'ORDER_BY'

const DEFAULT_SQL = `SELECT user_id, region, amount, create_time
FROM federated_orders
WHERE status = 'PAID'
ORDER BY
  amount DESC,
