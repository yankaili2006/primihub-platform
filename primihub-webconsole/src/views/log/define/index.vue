<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-button type="success" icon="el-icon-plus" @click="handleAdd">新增定义</el-button>
    </div>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="500px" :before-close="closeDialog">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="类型名称" prop="typeName">
          <el-input v-model="form.typeName" placeholder="请输入日志类型名称" maxlength="100" />
        </el-form-item>
        <el-form-item label="类型编码" prop="typeCode">
          <el-input v-model="form.typeCode" placeholder="请输入编码(如 LOGIN)" maxlength="64" :disabled="!!form.id" />
        </el-form-item>
        <el-form-item label="描述" prop="typeDesc">
          <el-input v-model="form.typeDesc" type="textarea" :rows="3" placeholder="描述信息" />
        </el-form-item>
...
import { getLogTypeList, saveLogType, deleteLogType } from '@/api/logType'

export default {
  name: 'LogTypeDefine',
  data() {
    return {
      loading: false, submitLoading: false, list: [],
      dialogVisible: false, dialogType: 'add', dialogTitle: '新增定义',
  ...
  methods: {
    async getList() {
      this.loading = true; try {      const res = await getLogTypeList()
      if (res.code === 0) this.list = res.result || []
      } catch(e) { console.error(e); this.$message({type:"error",message:"请求异常"}) } finally { } catch(e) { console.error(e); this.$message({type:"error",message:"请求异常"}) } finally { this.loading = false }
    },
    handleAdd() {
      this.dialogType = 'add'; this.dialogTitle = '新增定义'
      this.form = { id: null, typeName: '', typeCode: '', typeDesc: '', sortOrder: 0, status: 1 }
      this.dialogVisible = true
      this.$nextTick(() => { this.$refs.form && this.$refs.form.clearValidate() })
    },
    handleEdit(row) {
      this.dialogType = 'edit'; this.dialogTitle = '编辑定义'
      this.form = { ...row }
      this.dialogVisible = true
      this.$nextTick(() => { this.$refs.form && this.$refs.form.clearValidate() })
    },
    handleDelete(row) {
      this.$confirm('确定删除?', '提示', { type: 'warning' })
        .then(async () => { await deleteLogType({ id: row.id }); this.$message.success('删除成功'); this.getList() })
        .catch(() => {})
    },
    ...
    submitForm() {
      this.$refs.form.validate(async valid => {
        ...
        try {
          const res = await saveLogType(this.form)
          if (res.code === 0) {
