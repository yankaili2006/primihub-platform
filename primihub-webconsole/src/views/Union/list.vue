<template>
  <div class="container">
    ...
    <div class="organ-container">
      <el-button class="add-button" icon="el-icon-circle-plus-outline" type="primary" @click="toTaskPage">联邦求并</el-button>
      <div class="organ">
        <el-table
          :data="allDataUnionTask"
          class="table-list"
        >
          <el-table-column
            type="index"
          ...
          <el-table-column label="操作" fixed="right" min-width="120px" align="center">
            <template slot-scope="{row}">
              <p class="tool-buttons">
                ...
                <el-link type="danger" :disabled="row.taskState === 2" @click="delUnionTask(row)">删除</el-link>
              </p>
            </template>
          </el-table-column>
        </el-table>
...
import { getUnionTaskList, delUnionTask, cancelUnionTask, exportUnionLog } from '@/api/union'
...
export default {
  ...
  async created() {
    ...
    this.timer = window.setInterval(() => {
      setTimeout(this.getUnionTaskList(), 0)
    }, 3000)
  },
  destroyed() {
  ...
  methods: {
    ...
    toTaskPage() {
      this.$message.info('联邦求并任务创建页面开发中...')
      // this.$router.push({ name: 'UnionTask' })
    },
    toTaskDetailPage(id) {
      this.$message.info('联邦求并任务详情页面开发中...')
      // this.$router.push({ name: 'UnionDetail', params: { id } })
    },
    ...
    delUnionTask(row) {
      ...
      this.$confirm('此操作将永久删除该任务, 是否继续?', '提示', {
        ...
        delUnionTask({ taskId: row.taskId }).then(res => {
          if (res.code === 0) {
            const posIndex = this.allDataUnionTask.findIndex(item => item.taskId === row.taskId)
            if (posIndex !== -1) {
              this.allDataUnionTask.splice(posIndex, 1)
            }
            this.$message({
              message: '删除成功',
              type: 'success',
    ...
    async cancelTask(row) {
      ...
      if (res.code === 0) {
        const posIndex = this.allDataUnionTask.findIndex(item => item.taskId === row.taskId)
        this.allDataUnionTask[posIndex].taskState = 4
        this.$notify({
          message: '取消成功',
          type: 'success',
          duration: 1000
    ...
    getUnionTaskList() {
      ...
      getUnionTaskList(params).then(res => {
        ...
        this.total = total
        this.allDataUnionTask = data
        const result = this.allDataUnionTask.filter(item => item.taskState === 2)
        if (result.length === 0) {
          clearInterval(this.timer)
        }
      }).catch(error => {
    ...
    handleExportLog() {
      ...
      exportUnionLog(params).then(response => {
        const blob = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
        const url = window.URL.createObjectURL(blob)
        const link = document.createElement('a')
        link.href = url
        link.download = `联邦求并任务日志_${new Date().getTime()}.xlsx`
        link.click()
