<template>
  <div class="container">
    <el-page-header content="联邦求并任务详情" style="margin-bottom: 20px;" @back="$router.push({name:'UnionList'})" />
...
<script>
import { getUnionTaskDetails, downloadUnionTask } from '@/api/union'
...
export default {
  ...
  data() {
    return { loading: false, taskData: {}, taskId: this.$route.params.id, timer: null }
  },
  computed: {
  ...
  methods: {
    async fetchData() {
      this.loading = true
      try {
        const res = await getUnionTaskDetails({ taskId: this.taskId })
        if (res.code === 0) {
    ...
    async handleDownloadResult() {
      try {
        const res = await downloadUnionTask({ taskId: this.taskId })
        const blob = new Blob([res])
        const url = window.URL.createObjectURL(blob)
        const link = document.createElement('a')
        link.href = url; link.download = `求并结果_${this.taskData.taskName}_${Date.now()}.csv`
