<template>
  <div class="container">
    <el-row :gutter="16" class="stat-cards">
      <el-col :span="6"><div class="stat-card" :class="{ active: !query.status }" @click="filterByStatus('')"><div class="stat-num">{{ stats.total }}</div><div class="stat-label">需求总数</div></div></el-col>
      <el-col :span="6"><div class="stat-card" :class="{ active: query.status === '待处理' }" @click="filterByStatus('待处理')"><div class="stat-num warn">{{ stats.pending }}</div><div class="stat-label">待处理</div></div></el-col>
      <el-col :span="6"><div class="stat-card" :class="{ active: query.status === '进行中' }" @click="filterByStatus('进行中')"><div class="stat-num prog">{{ stats.inProgress }}</div><div class="stat-label">进行中</div></div></el-col>
      <el-col :span="6"><div class="stat-card" :class="{ active: query.status === '已完成' }" @click="filterByStatus('已完成')"><div class="stat-num done">{{ stats.completed }}</div><div class="stat-label">已完成</div></div></el-col>
    </el-row>
    <div class="search-area">
      ...
      <el-form :model="query" label-width="100px" :inline="true" @keyup.enter.native="search">
        ...
        <el-form-item label="需求类型">
          <el-select v-model="query.demandType" size="small" placeholder="请选择" clearable filterable>
    ...
    <div class="resource">
      <el-table :data="demandList" empty-text="暂无数据" border @selection-change="onSelectionChange">
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="800px" @close="handleDialogClose">
      <el-form ref="demandForm" :model="formData" :rules="formRules" :disabled="dialogMode === 'view'" label-width="120px">
        ...
        <el-form-item label="需求类型" prop="demandType">
          <el-select v-model="formData.demandType" placeholder="请选择需求类型" filterable allow-create default-first-option>
            <el-option v-for="t in typeOptions" :key="t" :label="t" :value="t" />
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async loadFilterOptions() {
      ...
      try {
        const res = await getDemandList({ pageNum: 1, pageSize: 500 })
        const list = ((res && res.result) || {}).list || []
        list.forEach(r => { if (r.demandType) set.add(r.demandType) })
      } catch (e) { /* ignore */ }
    ...
    handleBatchDelete() {
      ...
      this.$confirm(`确认删除选中的 ${ids.length} 条需求吗？`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async() => {
        let ok = 0, failed = 0
        for (const id of ids) {
          try { await deleteDemand(id); ok++ } catch (e) { failed++ }
        }
