<template>
  <div class="app-container">
    ...
    <el-row :gutter="20">
      <el-col :span="12">
        <el-card>
          <div slot="header"><span>创建特征相似度分析任务</span></div>
          <el-form ref="taskForm" :model="formData" :rules="formRules" label-width="120px">
            ...
            <el-form-item label="相似度算法" prop="algorithm">
              <el-select v-model="formData.algorithm" placeholder="请选择算法" style="width:100%;">
                <el-option label="余弦相似度" value="COSINE" />
                <el-option label="Jaccard" value="JACCARD" />
                <el-option label="皮尔逊相关" value="PEARSON" />
                <el-option label="欧氏距离" value="EUCLIDEAN" />
              </el-select>
            </el-form-item>
            <el-form-item label="目标特征字段" prop="targetFeatures">
              <el-select v-model="formData.targetFeatures" multiple placeholder="请选择目标特征字段" style="width:100%;">
                <el-option v-for="item in featureFieldList" :key="item" :label="item" :value="item" />
              </el-select>
            </el-form-item>
            <el-form-item label="相似度阈值" prop="similarityThreshold">
              <el-input-number v-model="formData.similarityThreshold" :min="0.0" :max="1.0" :step="0.01" :precision="2" style="width:100%;" />
            </el-form-item>
    ...
    <el-dialog title="任务详情" :visible.sync="detailVisible" width="600px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="相似度算法">{{ currentTask.algorithm }}</el-descriptions-item>
        <el-descriptions-item label="相似度阈值">{{ currentTask.similarityThreshold }}</el-descriptions-item>
...
const PREPROCESS_TYPE = 'FEATURE_SIMILARITY'
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        algorithm: [{ required: true, message: '请选择相似度算法', trigger: 'change' }]
      },
      dataResourceList: [
      ...
      featureFieldList: ['age', 'income', 'gender', 'region', 'score', 'amount'],
      taskList: [],
      listLoading: false,
      submitting: false,
      detailVisible: false,
  ...
  methods: {
    ...
    async handleDownload(row) {
      try {
        ...
        a.href = url
        a.download = `feature_similarity_${row.taskId}.csv`
        a.click()
