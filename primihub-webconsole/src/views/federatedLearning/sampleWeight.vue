<template>
  <div class="app-container">
    <el-page-header content="联邦学习样本加权" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-row :gutter="20">
      <el-col :span="12">
        <el-card>
          <div slot="header"><span>创建样本加权任务</span></div>
          <el-form ref="taskForm" :model="formData" :rules="formRules" label-width="120px">
            ...
            <el-form-item label="加权方式" prop="weightMethod">
              <el-select v-model="formData.weightMethod" placeholder="请选择加权方式" style="width:100%;">
                <el-option label="均等权重" value="EQUAL" />
                <el-option label="逆频率加权" value="INVERSE_FREQUENCY" />
                <el-option label="自定义权重" value="CUSTOM" />
              </el-select>
            </el-form-item>
            <el-form-item label="目标字段" prop="targetField">
              <el-select v-model="formData.targetField" placeholder="请选择目标字段" style="width:100%;">
                <el-option v-for="item in fieldList" :key="item" :label="item" :value="item" />
              </el-select>
            </el-form-item>
            <el-form-item label="加权系数" prop="weightCoefficient">
              <el-input-number
                v-model="formData.weightCoefficient"
                :min="0.1"
                :max="10"
                :step="0.1"
                :precision="1"
    ...
    <el-dialog title="任务详情" :visible.sync="detailVisible" width="600px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="加权方式">{{ currentTask.weightMethod }}</el-descriptions-item>
        <el-descriptions-item label="加权系数">{{ currentTask.weightCoefficient }}</el-descriptions-item>
...
const PREPROCESS_TYPE = 'SAMPLE_WEIGHT'
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        weightMethod: [{ required: true, message: '请选择加权方式', trigger: 'change' }],
      ...
      fieldList: ['label', 'target', 'y', 'class', 'category', 'outcome'],
      taskList: [],
      listLoading: false,
      submitting: false,
      detailVisible: false,
  ...
  methods: {
    ...
    async handleDownload(row) {
      try {
        ...
        a.href = url
        a.download = `sample_weight_${row.taskId}.csv`
        a.click()
