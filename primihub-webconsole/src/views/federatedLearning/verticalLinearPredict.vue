<template>
  <div class="app-container">
    <el-page-header content="纵向联邦线性回归预测" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-row :gutter="20">
      <el-col :span="12">
        <el-card>
          <div slot="header"><span>创建线性回归预测任务（真实联邦推理）</span></div>
          <el-form ref="taskForm" :model="formData" :rules="formRules" label-width="130px">
            ...
            <el-form-item label="训练好的模型" prop="taskId">
              <el-select v-model="formData.taskId" filterable placeholder="请选择训练成功的线性回归模型" style="width:100%;">
                <el-option v-for="m in modelTaskList" :key="m.taskId" :label="modelLabel(m)" :value="m.taskId" />
              </el-select>
              <div class="form-tip">仅列出训练成功的模型任务；没有可选项时请先完成一次线性回归训练</div>
            </el-form-item>
