<template>
  <div class="app-container">
    <el-page-header content="联邦学习线性回归预测（纵向）" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-row :gutter="20">
      <el-col :span="12">
        <el-card>
          <div slot="header"><span>创建纵向线性回归预测任务</span></div>
          <el-form ref="taskForm" :model="formData" :rules="formRules" label-width="130px">
            ...
            <el-form-item label="使用模型" prop="modelId">
              <el-select v-model="formData.modelId" placeholder="请选择已训练的线性回归模型" style="width:100%;">
            ...
            <el-form-item label="输出预测列名" prop="outputColumn">
              <el-input v-model="formData.outputColumn" placeholder="如: pred_value" />
            </el-form-item>
...
const PREPROCESS_TYPE = 'VFL_LINEAR_PREDICT'
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        outputColumn: [{ required: true, message: '请输入输出预测列名', trigger: 'blur' }]
      },
      dataResourceList: [
      ...
      modelList: [
        { modelId: 'linear_model_001', modelName: '线性回归模型_v1（2024-01-15）' },
        { modelId: 'linear_model_002', modelName: '线性回归模型_v2（2024-03-20）' }
      ],
      taskList: [],
      listLoading: false,
      submitting: false,
  ...
  methods: {
    ...
    async handleDownload(row) {
      try {
        ...
        a.href = url
        a.download = `vfl_linear_predict_${row.taskId}.csv`
        a.click()
