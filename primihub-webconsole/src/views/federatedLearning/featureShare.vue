<template>
  <div class="app-container">
    <el-page-header content="联邦学习特征分享" style="margin-bottom: 20px;" @back="$router.back()" />
    <el-alert title="该功能依赖平台联邦引擎的秘密共享组件，当前版本尚未接通：任务可登记但执行会明确失败，不会返回模拟结果。" type="warning" show-icon :closable="false" style="margin-bottom:20px;" />

    <el-row :gutter="20">
      <el-col :span="12">
        <el-card>
          <div slot="header"><span>创建特征分享任务</span></div>
          <el-form ref="taskForm" :model="formData" :rules="formRules" label-width="120px">
            ...
            <el-form-item label="分享方" prop="senderOrg">
              <el-select v-model="formData.senderOrg" placeholder="请选择分享方" style="width:100%;">
                <el-option label="机构A" value="ORG_A" />
                <el-option label="机构B" value="ORG_B" />
                <el-option label="机构C" value="ORG_C" />
              </el-select>
            </el-form-item>
            <el-form-item label="接收方" prop="receiverOrgs">
              <el-select v-model="formData.receiverOrgs" multiple placeholder="请选择接收方" style="width:100%;">
                <el-option label="机构A" value="ORG_A" />
                <el-option label="机构B" value="ORG_B" />
                <el-option label="机构C" value="ORG_C" />
              </el-select>
            </el-form-item>
            <el-form-item label="分享字段" prop="shareFields">
              <el-select v-model="formData.shareFields" multiple placeholder="请选择分享字段" style="width:100%;">
                <el-option v-for="item in fieldList" :key="item" :label="item" :value="item" />
              </el-select>
            </el-form-item>
            <el-form-item label="加密方式" prop="encryptType">
              <el-select v-model="formData.encryptType" placeholder="请选择加密方式" style="width:100%;">
                <el-option label="明文" value="PLAINTEXT" />
                <el-option label="同态加密" value="HOMOMORPHIC" />
                <el-option label="秘密共享" value="SECRET_SHARING" />
              </el-select>
            </el-form-item>
    ...
    <el-dialog title="任务详情" :visible.sync="detailVisible" width="600px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="分享方">{{ currentTask.senderOrg }}</el-descriptions-item>
        <el-descriptions-item label="加密方式">{{ currentTask.encryptType }}</el-descriptions-item>
...
const PREPROCESS_TYPE = 'FEATURE_SHARE'
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        senderOrg: [{ required: true, message: '请选择分享方', trigger: 'change' }],
        receiverOrgs: [{ required: true, message: '请选择接收方', trigger: 'change' }],
        shareFields: [{ required: true, message: '请选择分享字段', trigger: 'change' }],
      ...
      fieldList: ['age', 'income', 'gender', 'region', 'score', 'amount', 'category'],
      taskList: [],
      listLoading: false,
      submitting: false,
      detailVisible: false,
  ...
  methods: {
    ...
    async handleDownload(row) {
      try {
        ...
        a.href = url
        a.download = `feature_share_${row.taskId}.csv`
        a.click()
