<template>
  <div class="app-container">
    <el-page-header content="联邦学习数据转换" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-row :gutter="20">
      <el-col :span="12">
        <el-card>
          <div slot="header"><span>创建数据转换任务</span></div>
          <el-form ref="taskForm" :model="formData" :rules="formRules" label-width="130px">
            ...
            <el-form-item label="转换操作" prop="transformOps">
              <el-checkbox-group v-model="formData.transformOps">
                <el-checkbox label="TYPE_CAST">类型转换</el-checkbox>
                <el-checkbox label="MISSING_VALUE">缺失值处理</el-checkbox>
                <el-checkbox label="NORMALIZE">归一化</el-checkbox>
                <el-checkbox label="STANDARDIZE">标准化</el-checkbox>
                <el-checkbox label="DISCRETIZE">离散化</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
    ...
    <el-dialog title="任务详情" :visible.sync="detailVisible" width="600px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="转换操作">{{ (currentTask.transformOps || []).join(', ') }}</el-descriptions-item>
  ...
  methods: {
    ...
    async handleDownload(row) {
      try {
        ...
        a.href = url
        a.download = `data_transform_${row.taskId}.csv`
        a.click()
