<template>
  <div class="app-container">
    <el-page-header content="联邦学习特征装仓" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-row :gutter="20">
      <el-col :span="12">
        <el-card>
          <div slot="header"><span>创建特征装仓任务</span></div>
          <el-form ref="taskForm" :model="formData" :rules="formRules" label-width="120px">
            ...
            <el-form-item label="特征仓库名称" prop="warehouseName">
              <el-input v-model="formData.warehouseName" placeholder="请输入特征仓库名称" />
            </el-form-item>
            <el-form-item label="特征版本" prop="featureVersion">
              <el-input v-model="formData.featureVersion" placeholder="如: v1.0.0" />
            </el-form-item>
            <el-form-item label="特征字段" prop="featureFields">
              <el-select v-model="formData.featureFields" multiple placeholder="请选择特征字段" style="width:100%;">
                <el-option v-for="item in fieldList" :key="item" :label="item" :value="item" />
              </el-select>
            </el-form-item>
            <el-form-item label="存储格式" prop="storageFormat">
              <el-select v-model="formData.storageFormat" placeholder="请选择存储格式" style="width:100%;">
    ...
    <el-dialog title="任务详情" :visible.sync="detailVisible" width="600px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="仓库名称">{{ currentTask.warehouseName }}</el-descriptions-item>
        <el-descriptions-item label="特征版本">{{ currentTask.featureVersion }}</el-descriptions-item>
        <el-descriptions-item label="存储格式">{{ currentTask.storageFormat }}</el-descriptions-item>
...
const PREPROCESS_TYPE = 'FEATURE_WAREHOUSE'
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        warehouseName: [{ required: true, message: '请输入特征仓库名称', trigger: 'blur' }],
        featureVersion: [{ required: true, message: '请输入特征版本', trigger: 'blur' }],
        featureFields: [{ required: true, message: '请选择特征字段', trigger: 'change' }],
        storageFormat: [{ required: true, message: '请选择存储格式', trigger: 'change' }]
      },
      dataResourceList: [
      ...
      fieldList: ['age', 'income', 'gender', 'region', 'score', 'amount', 'category', 'tenure'],
      taskList: [],
      listLoading: false,
      submitting: false,
      detailVisible: false,
  ...
  methods: {
    ...
    async handleDownload(row) {
      try {
        ...
        a.href = url
        a.download = `feature_warehouse_${row.taskId}.parquet`
        a.click()
