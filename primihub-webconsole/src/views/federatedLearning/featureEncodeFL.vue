<template>
  <div class="app-container">
    <el-page-header content="联邦学习特征编码" style="margin-bottom: 20px;" @back="$router.back()" />

    <el-row :gutter="20">
      <el-col :span="12">
        <el-card>
          <div slot="header"><span>创建特征编码任务</span></div>
          <el-form ref="taskForm" :model="formData" :rules="formRules" label-width="120px">
            ...
            <el-form-item label="编码方法" prop="encodeMethod">
              <el-select v-model="formData.encodeMethod" placeholder="请选择编码方法" style="width:100%;">
                <el-option label="OneHot编码" value="ONEHOT" />
                <el-option label="Label编码" value="LABEL" />
                <el-option label="Binary编码" value="BINARY" />
              </el-select>
            </el-form-item>
            <el-form-item label="编码字段" prop="encodeFields">
              <el-select v-model="formData.encodeFields" multiple placeholder="请选择编码字段" style="width:100%;">
    ...
    <el-dialog title="任务详情" :visible.sync="detailVisible" width="600px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="编码方法">{{ currentTask.encodeMethod }}</el-descriptions-item>
        <el-descriptions-item label="编码字段">{{ (currentTask.encodeFields || []).join(', ') }}</el-descriptions-item>
...
const PREPROCESS_TYPE = 'FL_FEATURE_ENCODE'
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        encodeFields: [{ required: true, message: '请选择编码字段', trigger: 'change' }]
      },
      dataResourceList: [
      ...
      fieldList: ['gender', 'region', 'category', 'level', 'type', 'status'],
      taskList: [],
      listLoading: false,
      submitting: false,
      detailVisible: false,
  ...
  methods: {
    ...
    async handleDownload(row) {
      try {
        ...
        a.href = url
        a.download = `feature_encode_${row.taskId}.csv`
        a.click()
