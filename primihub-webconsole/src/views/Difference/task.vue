<template>
  <div class="container">
    <el-page-header content="联邦求差任务" style="margin-bottom: 20px;" @back="$router.push({name:'DifferenceList'})" />
    ...
    <div class="step" v-show="stepActive === 0">
      <div class="inner-con">
        <el-form ref="form" :model="formData" :rules="rules" label-width="140px">
          ...
          <el-form-item label="求差结果名称" prop="resultName">
            <el-input v-model="formData.resultName" placeholder="默认自动填充为任务名称" />
          </el-form-item>
          <el-form-item label="求差方向" prop="differenceDirection">
            <el-select v-model="formData.differenceDirection" placeholder="请选择求差方向">
              <el-option label="本机构 - 对方" :value="0">
                <span style="margin-right: 8px;">本机构 - 对方</span>
                <span style="font-size:12px;color:#909399;">(A - B)</span>
              </el-option>
              <el-option label="对方 - 本机构" :value="1">
                <span style="margin-right: 8px;">对方 - 本机构</span>
                <span style="font-size:12px;color:#909399;">(B - A)</span>
              </el-option>
            </el-select>
            <div class="form-tip">求差方向决定了差集计算的方向，即 A 有而 B 没有的数据，或 B 有而 A 没有的数据</div>
          </el-form-item>
        </el-form>
      </div>
...
<script>
import { saveDataDifference } from '@/api/difference'
...
export default {
  ...
  data() {
    return {
      ...
      formData: {
        ...
        resultName: '', resultOrganIds: '', tag: 0, differenceDirection: 0,
        teeOrganId: '', remarks: ''
      },
      ...
      rules: {
        ...
        differenceDirection: [{ required: true, message: '请选择求差方向' }],
  ...
  computed: {
    centerImg() { return require('@/assets/diffsection.svg') }
  },
  async created() {
  ...
  methods: {
    autoFillResultName() {
      if (!this.formData.resultName) this.formData.resultName = this.formData.taskName + '_差集结果'
    },
    getOrganName(id) {
    ...
    handleSubmit() {
      this.$refs.form.validate(async valid => {
        ...
        if (res.code === 0) {
          this.$notify({ title: '创建成功', message: '联邦求差任务已创建，正在跳转详情页...', type: 'success', duration: 2000 })
          setTimeout(() => this.$router.push({ name: 'DifferenceDetail', params: { id: res.result } }), 1000)
        }
      })
    }
  }
