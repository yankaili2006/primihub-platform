<template>
  <div class="log-page">
    <!-- 模块上下文才显示算法导航；项目 tab 内注入 projectId，隐藏跳走式导航 -->
    <FederatedQueryNav v-if="!projectId" />
    <el-card>
      <div slot="header"><span>联邦查询记录{{ projectId ? '（本项目）' : '' }}</span></div>
...
export default {
  ...
  props: {
    // 项目上下文：传入则记录列表按该项目过滤（后端 project_id 端到端支持）
  ...
  methods: {
    ...
    async search() {
      this.loading = true
      try {
        const { code, result } = await getFederatedQueryList(this._wp({
