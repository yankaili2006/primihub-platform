...
export default {
  name: 'FsCenter',
  props: {
    // 项目上下文：传入则任务列表/创建均归属该项目（QueryReq/Req 后端原生支持 projectId）
  ...
  methods: {
    _wp(o) {
      return (this.projectId != null && this.projectId !== '') ? { ...o, projectId: this.projectId } : o
    },
    async fetchData() {
      this.loading = true
      try {
        const res = await getFederatedStatisticsList(this._wp(this.queryForm))
        if (res && res.code === 0) {
    ...
    async handleCreateSubmit() {
      this.$refs.createForm.validate(async(valid) => {
        if (valid) {
          try {
            const res = await createFederatedStatistics(this._wp(this.createFormData))
            if (res && res.code === 0) {
              this.$message.success('联邦统计任务创建成功')
              this.createDialogVisible = false
              this.fetchData()
            } else {
              this.$message.error('创建失败: ' + ((res && res.msg) || '接口返回异常'))
              // 不造假：创建失败绝不向表格塞伪造行
            }
          } catch (error) {
