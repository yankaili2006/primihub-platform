...
# 默认配置
DEFAULT_IMAGE_NAME="primihub-webconsole"
DEFAULT_TAG="latest"
...
usage() {
    ...
    echo "示例:"
    echo "  $0                          # 使用默认配置构建镜像"
    echo "  $0 -n my-webconsole -t v1.0 # 构建名为 my-webconsole:v1.0 的镜像"
...
check_build_artifacts() {
    if [[ "$DOCKERFILE" == "Dockerfile" ]]; then
        ...
        echo "找到构建产物: dist/ 目录"
        echo "构建文件统计: $(find dist/ -type f | wc -l) 个文件"
    fi
}
...
build_image() {
    ...
    if [[ $? -eq 0 ]]; then
        ...
        echo ""
        echo "运行容器:"
        echo "  docker run -d -p 80:80 --name primihub-webconsole $full_image_name"
        echo ""
        echo "查看镜像:"
