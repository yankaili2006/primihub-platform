    ...
    if result.get('code') == 0:
        ...
        if projects:
            ...
            psi_data = {
                ...
                # 发起方信息
                "ownOrganId": organ_id,
                "ownResourceId": "2",  # 我们创建的资源ID
                "ownKeyword": "user_id",  # CSV文件中的列名

                # 协作方信息（单方测试时使用相同的机构和资源）
                ...
                "outputContent": 0,  # 输出内容类型
                "outputNoRepeat": 1,  # 去重
                "outputFilePathType": 0,  # 输出文件路径类型
            ...
            print(f"   发起方资源: {psi_data['ownResourceId']}")
            print(f"   匹配字段: {psi_data['ownKeyword']}")
                        ...
                        if result.get('code') == 0:
                            ...
                            if tasks:
                                ...
                                if response.text:
                                    result = response.json()
                                    if result.get('code') == 0:
                                        psi_tasks = result.get('result', {}).get('list', [])
