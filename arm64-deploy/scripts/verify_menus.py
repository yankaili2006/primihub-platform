...
def main():
    ...
    # ③ v1.8.0 按钮权限(zz-button-perms-comprehensive.sql 补齐的 13 个 authType=3 码;
    #    前端据 buttonPermissionList=授权树里 authType===3 的 authCode 渲染这些按钮)
    NEED_BTNS = ["TenantAdd", "TenantEdit", "TenantDelete", "TenantFreeze",
                 "ResourceDelete", "ResourceEdit", "UnionResourceDetail",
                 "ModelCreate", "ProjectDetail", "UserFreeze",
                 "WhitelistAdd", "WhitelistEdit", "WhitelistConfigEdit"]
    miss_btn = [c for c in NEED_BTNS if c not in granted]
    passed = (ok if not miss_btn else fail)("v1.8.0 按钮权限 %d/%d%s"
              % (len(NEED_BTNS) - len(miss_btn), len(NEED_BTNS),
