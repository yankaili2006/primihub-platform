-- zz-v18-tables.sql
-- 补齐 v1.8.0 后端所需但部署 privacy*.sql 缺失的 18 张表(+种子)。
-- 根因: privacy*.sql dump 早于/未含 v1.8.0 表(tenant/whitelist/log/node-approval);
--   后端(TenantPrimarydbRepositoryMapper 等)真查这些表 → 全新部署打开对应页面会报错。
--   (.130 生产已有这些表; 仅全新离线部署缺。)
-- 表来自 primihub-service/script/{tenant,whitelist,log_management,create_node_tables}.sql。
-- CREATE TABLE IF NOT EXISTS + 种子 ON DUPLICATE/INSERT IGNORE = 幂等; zz- 在 privacy*.sql 后加载。
