-- zz-button-perms-comprehensive.sql
-- 补齐前端所有按钮权限(authType=3)+授权超管，修复 v1.8.0 多页按钮不显示。
-- 根因: 部署 privacy*.sql 只 seed 菜单页/页面解锁(authType=2)，缺 authType=3 按钮权限;
--   前端 buttonPermissionList=授权树里 authType===3 的 authCode → 缺则按钮不渲染。
-- 覆盖 9 个缺失按钮(资源删/改·联合资源详情·创建模型·项目详情·冻结用户·白名单增/改·白名单配置改)
--   + 4 个租户按钮(取代原 zz-tenant-button-perms.sql)，并把全部 32 个前端按钮码授予 role_id=1。
-- 幂等(NOT EXISTS 判重),zz- 前缀在 privacy*.sql 之后加载;也可对运行库直接执行(后清 Redis sys_auth:bfs_list 重登录)。
-- 后端为 token 鉴权(无 per-url 权限校验),按钮可见性只需 authType=3+授权。

-- ======== privacy1 ========
USE `privacy1`;
-- === comprehensive button-perm seed for ONE privacy DB (idempotent) ===
...
-- resolve parent menu (target=_btn_map temp; sys_auth only in subquery -> allowed)
...
-- insert missing authType=3 buttons (parent resolved from temp table)
...
-- set full_path via join to temp (no sys_auth self-subquery -> no error 1093)
