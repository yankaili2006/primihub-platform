...
# 默认配置
DEFAULT_IMAGE_NAME="primihub-service"
DEFAULT_TAG="latest"
...
usage() {
    ...
    echo "示例:"
    echo "  $0                          # 使用默认配置构建镜像"
    echo "  $0 -n my-service -t v1.0    # 构建名为 my-service:v1.0 的镜像"
...
check_build_artifacts() {
    if [[ "$DOCKERFILE" == "Dockerfile" ]]; then
        local app_jar=$(find application/target -name "*-SNAPSHOT.jar" -type f 2>/dev/null | head -n 1)
        local gateway_jar=$(find gateway/target -name "*-SNAPSHOT.jar" -type f 2>/dev/null | head -n 1)
        
        if [[ -z "$app_jar" ]] || [[ -z "$gateway_jar" ]]; then
...
build_image() {
    ...
    if [[ $? -eq 0 ]]; then
        ...
        echo ""
        echo "运行容器:"
        echo "  docker run -d -p 8080:8080 --name primihub-service $full_image_name"
        echo ""
        echo "查看镜像:"
