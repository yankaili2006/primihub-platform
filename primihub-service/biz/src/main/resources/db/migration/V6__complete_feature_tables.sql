-- V6__complete_feature_tables.sql — 平台完整功能表结构(幂等 CREATE TABLE IF NOT EXISTS)。
-- 背景(runtime 实测 39.105.61.64): 大量功能表(存证/联邦查询·统计·分析·学习/求差·求并/
-- 数据需求/接口管理/监控告警/项目权限 等 ~40 张)只存在于 application/resources/schema-mysql.sql,
-- 从未进入 Flyway/部署 initsql, 导致 fresh 部署缺表 -> 多个模块运行时 'Table doesn't exist'。
-- 本迁移把 schema-mysql.sql 的全部建表纳入 Flyway。已存在的表被 IF NOT EXISTS 跳过, 安全。
-- 与 V3(v18_tables)/V4(scene_imported_data)/V5(scene 基础表)幂等叠加。
