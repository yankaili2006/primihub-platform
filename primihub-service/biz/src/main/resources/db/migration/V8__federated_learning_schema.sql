-- V7__federated_learning_schema.sql
-- 补齐联邦学习主表 federated_learning + federated_learning_task（Flyway 之前遗漏，导致
-- getTaskList 查询 privacy{N}.federated_learning 报 "Table doesn't exist" → 前端"查询失败"，
-- 即缺陷 D41/D38）。幂等（IF NOT EXISTS），对已建表环境安全。
-- 根因：该 DDL 原在 database_schema_federated_learning.sql，但从未接入 db/migration 迁移链。
-- ========================================
-- 联邦学习功能数据库表
-- ========================================
